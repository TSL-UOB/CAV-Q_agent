...
def MASrender(simTime, nA, agentState, score, nExp):
	...
	#======================================
	#print score
	text = 'Accuracy = ' + str(int(score)) + ' / ' + str(int(nExp+1))	
	color = (0,0,255)
	font = cv2.FONT_HERSHEY_SIMPLEX
	y = gridH - 2
	x =  7
	...
	x2, y2  = (int((x+15)*env.scale-w/2), int((y-0.9)*env.scale+h/2))
...
nTests = 50					# Number of experiements to run
	...
	while(not(done)) and (nExp <= nTests-1):
		...
		if plotAccuracy:
			...
			if (Acounter>2) and ((nExp>nTests-2) or (nExp%display_chart_modulo==0)):
				...
				plt.title('Test Method: %s' % agentBehaviour)
				plt.grid(True)
				plt.ylim((0,100))
				Afig.canvas.draw()
				...
				if (nExp == nTests-1):	
					plt.savefig('plots/Accuracy_%s.png' % ts)
